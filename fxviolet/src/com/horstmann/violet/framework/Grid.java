...
public class Grid
{
   ...
   */
   public void draw(Graphics2D g2, Rectangle bounds)
   {
      ...
      for (double x = bounds.getX(); x < bounds.getX() + bounds.getWidth(); x += gridx)
         g2.draw(new Line(x, bounds.getY(), x, bounds.getY() + bounds.getHeight()));
      for (double y = bounds.getY(); y < bounds.getY() + bounds.getHeight(); y += gridy)
         g2.draw(new Line(bounds.getX(), y, bounds.getX() + bounds.getWidth(), y));
