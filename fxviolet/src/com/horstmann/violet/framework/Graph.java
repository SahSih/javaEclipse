...
public abstract class Graph implements Serializable
{
   ...
   */
   ...
   {
      Rectangle r = minBounds;
      for (int i = 0; i < nodes.size(); i++)
      {
         Node n = (Node)nodes.get(i);
         Rectangle b = n.getBounds();
         //
         Shape rect = Shape.union(r, b);
         Bounds bounds = rect.getLayoutBounds();
         //
         if (r == null) r = b;
         //else r.add(b);
         else
            r = new Rectangle(bounds.getMinX(), bounds.getMinY(), bounds.getWidth(), bounds.getHeight());
      ...
      for (int i = 0; i < edges.size(); i++)
      {
         Edge e = (Edge)edges.get(i);
         //
         Shape rect = Shape.union(r, e.getBounds(g2));
         Bounds bounds = rect.getLayoutBounds();
         //r.add(e.getBounds(g2));
         r = new Rectangle(bounds.getMinX(), bounds.getMinY(), bounds.getWidth(), bounds.getHeight());
         //
      }
      return r == null ? new Rectangle() : new Rectangle(r.getX(), r.getY(), 
            r.getWidth() + AbstractNode.SHADOW_GAP, r.getHeight() + AbstractNode.SHADOW_GAP);
   }
   
   public Rectangle getMinBounds() { return minBounds; }
   public void setMinBounds(Rectangle newValue) { minBounds = newValue; }
         ...
         DefaultPersistenceDelegate()
         {
               ...
               Encoder out) 
            {
               ...
               for (int i = 0; i < g.nodes.size(); i++)
               {
                  ...
                  Rectangle bounds = n.getBounds();
                  Point2D p = new Point2D(bounds.getX(),
                     bounds.getY());
                  out.writeStatement(
   ...
   */
   ...
   private transient Rectangle minBounds;
}
