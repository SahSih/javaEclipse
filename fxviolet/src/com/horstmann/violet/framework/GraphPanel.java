...
public class GraphPanel extends JPanel
{
   ...
   public GraphPanel(ToolBar aToolBar)
   {
      ...
      addMouseListener(new MouseAdapter()
      {
         ...
         {
            requestFocus();
            final Point2D mousePoint = new Point2D(event.getX() / zoom,
                  event.getY() / zoom);
         ...
         {
            Point2D mousePoint = new Point2D(event.getX() / zoom,
                  event.getY() / zoom);
            Object tool = toolBar.getSelectedTool();
      ...
      addMouseMotionListener(new MouseMotionAdapter()
      {
         public void mouseDragged(MouseEvent event)
         {
            ...
            {               
               ...
               while (iter.hasNext())
               {
                  ...
                  if (selected instanceof Node)
                  {
                     ...
                     //
                     Shape sbounds = Shape.union(sbounds, n.getBounds());
                     Bounds bbounds = sbounds.getLayoutBounds();
                     bounds = new Rectangle(bbounds.getMinX(), bbounds.getMinY(), bbounds.getWidth(), bbounds.getHeight());
                     //
                  }
               }
            ...
            else if (dragMode == DRAG_LASSO)
            {
               ...
               Rectangle lasso = new Rectangle(Math.min(x1, x2), 
   ...
   public void paintComponent(Graphics g)
   {
      ...
      Rectangle graphBounds = graph.getBounds(g2);
      if (!hideGrid) grid.draw(g2, new Rectangle(0, 0, 
            Math.max(bounds.getX()+bounds.getWidth() / zoom, graphBounds.getX() + graphBounds.getWidth()), 
            Math.max(bounds.getY()+bounds.getHeight() / zoom, graphBounds.getY() + graphBounds.getHeight())));
      graph.draw(g2, grid);
      ...
      while (iter.hasNext())
      {
         ...
         else if (selected instanceof Node)
         {
            ...
            drawGrabber(g2, grabberBounds.getX(), grabberBounds.getY()+grabberBounds.getHeight());
            drawGrabber(g2, grabberBounds.getX()+grabberBounds.getWidth(), grabberBounds.getY());
            drawGrabber(g2, grabberBounds.getX()+grabberBounds.getWidth(), grabberBounds.getY()+grabberBounds.getHeight());
         }
         else if (selected instanceof Edge)
         {
            Line line = ((Edge) selected).getConnectionPoints();
            drawGrabber(g2, line.getStartX(), line.getStartY());
            drawGrabber(g2, line.getEndX(), line.getEndY());
         }
      }
      ...
      if (dragMode == DRAG_RUBBERBAND)
      {
         ...
         g2.setColor(PURPLE);
         g2.draw(new Line(mouseDownPoint, lastMousePoint));
   ...
   public static void drawGrabber(Graphics2D g2, double x, double y)
   {
      ...
      Color oldColor = g2.getColor();
      g2.setColor(PURPLE);
      g2.fill(new Rectangle(x - SIZE / 2, y - SIZE / 2, SIZE, SIZE));
      g2.setColor(oldColor);
   }

   public Dimension getPreferredSize()
   {
      Rectangle bounds = graph.getBounds((Graphics2D) getGraphics());
      return new Dimension((int) (zoom * bounds.getX()+bounds.getWidth()),
            (int) (zoom * bounds.getY()+bounds.getHeight()));
   }
   ...
   public void selectNext(int n)
   {
      ...
      java.util.Collections.sort(selectables, new java.util.Comparator()
      {
         public int compare(Object obj1, Object obj2)
         {
            ...
            {
               Rectangle bounds = ((Node) obj1).getBounds();
               x1 = bounds.getX();
               y1 = bounds.getY();
            }
            else
            {
               Point2D p = new Point2D(((Edge) obj1).getConnectionPoints().getStartX(), 
                     ((Edge) obj2).getConnectionPoints().getStartY());
            ...
            {
               Rectangle bounds = ((Node) obj2).getBounds();
               x2 = bounds.getX();
               y2 = bounds.getY();
            }
            else
            {
               Point2D p = new Point2D(((Edge) obj2).getConnectionPoints().getStartX(), 
                     ((Edge) obj2).getConnectionPoints().getStartY());
